...
<!--
	var loc_url_del = "ux-admin/ManageGameDocument/Delete/";
//-->
</script>
				...
				<div class="col-sm-10 col-sm-offset-1">
					<form method="post" action="" enctype="multipart/form-data">
						<div class="form-group col-sm-12">
							...
							<input type="hidden" name="id" value="<?php if(isset($_GET['tab'])){ echo $resultdoc->GameDoc_ID; } ?>" >
							<label>Title</label>
							<input name="doctitle" id="doctitle" class="form-control" value="<?php if(isset($_GET['tab'])){ echo $resultdoc->GameDoc_Title; } ?>">
						</div>
						<div class="form-group col-sm-12">		
							<label>Name</label>
							<input name="docname" type="file" id="docname" class="form-control" value="<?php if(isset($_GET['tab'])){ echo $resultdoc->GameDoc_Name; } ?>">
						</div>
						<div class="form-group text-center">
							<?php if(isset($_GET['tab'])){ ?>
								<button class="btn btn-primary" type="submit" name="submit" value="Update">Update</button>
								<button class="btn btn-primary" type="button" onclick="window.location='<?php echo site_root."ux-admin/ManageGameDocument/Edit/".base64_encode($result->Game_ID); ?>';">Cancel</button>
							<?php }else{ ?>
								<button class="btn btn-primary" id="submit" type="submit" name="submit" value="Submit">Submit</button>
								<!--<button class="btn btn-primary" type="button" onclick="window.location='<?php echo site_root."ux-admin/ManageGame"; ?>';">Cancel</button>-->
							<?php } ?>
									...
									<?php							
										if($object->num_rows > 0){	
											...
											while($row = $object->fetch_object()){ ?>
												<tr>										
													<th><?php echo $i;?></th>
													<td><?php echo ucfirst($row->GameDoc_Title);?></td>
													<td><?php echo ucfirst($row->GameDoc_Name);?></td>
													<td class="text-center">
														<a href="<?php echo site_root."ux-admin/ManageGameDocument/Edit/".base64_encode($row->GameDoc_GameID)."/tab/".base64_encode($row->GameDoc_ID);?>"
															title="Edit">
															<span class="fa fa-pencil"></span>
														</a>
														<a href="javascript:void(0);" class="dl_btn" id="<?php echo $row->GameDoc_ID; ?>"
															title="Delete">
