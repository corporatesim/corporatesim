<!-- <?php // echo "<pre>"; while($row = $chartList->fetch_object()){ print_r($row); } exit(); ?> -->
		...
		<div class="col-sm-12">
			<ul class="breadcrumb">
				<li class="completed"><a
					...
					<li class="active"><a href="<?php echo site_root."ux-admin/chartComp"; ?>">Manage Component Chart</a></li>
		...
		<div class="col-sm-12">
			<form method="POST" action="" id="siteuser_frm" name="siteuser_frm">
				...
				<div class="row">
					...
					<div class="col-md-4" id="scenarioSelect">
						<label><span class="alert-danger">*</span>Select Scenario</label> <span class="text-right text-danger" id="showMsg"></span>
						<select class="form-control" name="scen_id" id="scen_id" required="">
							<option value="">-- SELECT SCENARIO--</option>
							<?php while($row = $scenario->fetch_object()){ print_r($row); ?>
				...
				<div class="row">
					<div class="col-sm-12">
						<div class="form-group text-center">
							...
							<?php }else{?>
								<button type="button" id="siteuser_btn" class="btn btn-primary" value="Submit"> Submit </button>
 ...
 $('#scen_id').change( function(){
	...
	$('#showMsg').text('To See Dropdown Option For Area, Component, SubComponent Click On Its Box Accordingly');

	$.ajax({
		url : site_root + "ux-admin/model/ajax/populate_dropdown.php",
		type: "POST",
		data: { areaScenId: scen_id, type_dropdown:'dorpdown', areaGameId: game_id },
		success: function(data){
			console.log(data);
			$('#area_id').html(data);
		}
	...
	$.ajax({
		url : site_root + "ux-admin/model/ajax/populate_dropdown.php",
		type: "POST",
		data: { compScenId: scen_id,type_dropdown:'dorpdown', compGameId: game_id },
		success: function(data){
			$('#comp_id').html(data);
		}
	});		
	...
	$.ajax({
		url : site_root + "ux-admin/model/ajax/populate_dropdown.php",
		type: "POST",
		data: { subcompScenId: scen_id, type_dropdown:'dorpdown', subcompGameId: game_id },
		success: function(data){
			$('#subcomp_id').html(data);
		}
	});		
