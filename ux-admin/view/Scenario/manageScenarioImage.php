...
<!--
	var loc_url_del = "ux-admin/ManageScenarioImage/Delete/";
//-->
</script>
				...
				<div class="col-sm-10 col-sm-offset-1">
					<form method="post" action="" enctype="multipart/form-data">
						<div class="form-group col-sm-12">
							...
							<label>Title</label>
							<input name="imgtitle" id="imgtitle" class="form-control" value="<?php if(isset($_GET['tab'])){ echo $resultdoc->ScenImg_Title; } ?>">
						</div>
						<div class="form-group col-sm-12">		
							<label>Name</label>
							<input name="imgname" type="file" id="imgname" class="form-control" value="<?php if(isset($_GET['tab'])){ echo $resultdoc->ScenImg_Name; } ?>">
						</div>
						<div class="form-group col-sm-12">		
							<label>Comments</label>
							<textarea id="imgcomments" name="imgcomments" class="form-control"><?php if(isset($_GET['tab'])){ echo $resultdoc->ScenImg_Comments; } ?></textarea>
						</div>
						<div class="form-group text-center">
							<?php if(isset($_GET['tab'])){ ?>
								<button class="btn btn-primary" type="submit" name="submit" value="Update">Update</button>
								<button class="btn btn-primary" type="button" onclick="window.location='<?php echo site_root."ux-admin/ManageScenarioImage/Edit/".base64_encode($resultdoc->ScenImg_ScenID); ?>';">Cancel</button>
							<?php }else{ ?>
									...
									<?php							
										if($object->num_rows > 0){	
											...
											while($row = $object->fetch_object()){ ?>
												<tr>										
													<th><?php echo $i;?></th>
													<td><?php echo ucfirst($row->ScenImg_Title);?></td>
													<td><?php echo ucfirst($row->ScenImg_Name);?></td>
													<td class="text-center">
														<a href="<?php echo site_root."ux-admin/ManageScenarioImage/Edit/".base64_encode($row->ScenImg_ScenID)."/tab/".base64_encode($row->ScenImg_ID);?>"
															title="Edit">
															<span class="fa fa-pencil"></span>
														</a>
														<a href="javascript:void(0);" class="dl_btn" id="<?php echo $row->ScenImg_ID; ?>"
															title="Delete">
