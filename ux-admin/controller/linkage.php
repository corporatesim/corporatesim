	...
	if(isset($_GET['link']))
	{
		...
		if($error =="Error_Formula")
		{
			...
			$linkdetails = (object) array(
				...
				'SubLink_Competence_Performance' => $_POST['SubLink_Competence_Performance'],
				'SubLink_Order'                  => $_POST['order'],
				'SubLink_ShowHide'               => $_POST['ShowHide'],
				'SubLink_FormulaID'              => $_POST['inputType']=='formula' ? $_POST['formula_id']: null,
				'SubLink_AdminCurrent'           => $_POST['inputType']=='admin' ? (empty($_POST['current'])?0.00:$_POST['current']):null,
				'SubLink_AdminLast'              => $_POST['inputType']=='admin' ? (empty($_POST['last'])?0.00:$_POST['last']):null,
				...
				'SubLink_InputBackgroundColor'   => $_POST['SubLink_InputBackgroundColor'],
...
elseif(isset($_GET['linkdel']))
{
  ...
  if($object2->num_rows>0)
  {
    ...
    //searchin item mapping for this sublink id
    $item_Mapping_Sql    = "SELECT GIM.* FROM GAME_ITEMS_MAPPING as GIM WHERE Cmap_SublinkId=".$sublinkid;
    $item_Mapping_Object = $functionsObj->RunQueryFetchCount($item_Mapping_Sql);

    if($item_Mapping_Object > 0)
    {
      //this sublink linked with item so, give an alert
      $_SESSION['msg']     = "Cannot be deleted as it is linked to Process Owner reports";
      $_SESSION['type[0]'] = "inputError";
      $_SESSION['type[1]'] = "has-error";
      header("Location: ".site_root."ux-admin/linkage/link/".$linkid);
      exit(0);
