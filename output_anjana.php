...
/*$sql ="SELECT gi.input_Current,gi.input_key,gi.input_user,gp.Outcome_CompId,gp.Outcome_MinVal,gp.Outcome_MaxVal, gp.Outcome_Result,gr.Outcome_Name,gcomp.Comp_Name FROM `game_input` gi JOIN game_personalize_outcome gp JOIN game_outcome_result gr ON gr.Outcome_ResultID = gp.Outcome_Result  JOIN GAME_COMPONENT gcomp ON gcomp.Comp_ID = gp.Outcome_CompId WHERE input_sublinkid IN( SELECT SubLink_ID FROM `game_linkage_sub` WHERE SubLink_LinkID IN( SELECT Link_ID FROM `GAME_LINKAGE` WHERE Link_ID= $linkid)AND SubLink_Type=1 AND SubLink_ShowHide=1) AND gi.input_current >= gp.Outcome_MinVal AND gi.input_current <= gp.Outcome_Maxval  AND gi.input_user=$userid AND gp.Outcome_GameId=$gameid";*/


$sql ="SELECT gl.SubLink_ID,gp.*,gi.input_key,gi.input_current,gr.Outcome_Name,gcomp.Comp_Name FROM game_personalize_outcome gp LEFT JOIN GAME_LINKAGE_SUB gl ON gl.SubLink_CompID=gp.Outcome_CompId AND gl.SubLink_LinkID=gp.Outcome_LinkId LEFT JOIN game_outcome_result gr ON gr.Outcome_ResultID = gp.Outcome_FileType LEFT JOIN GAME_COMPONENT gcomp ON gcomp.Comp_ID = gp.Outcome_CompId LEFT JOIN GAME_INPUT gi ON gi.input_sublinkid=gl.SubLink_ID WHERE gp.Outcome_GameId=$gameid AND gi.input_user=$userid AND gl.SubLink_Type=1 AND gl.SubLink_SubCompID=0  AND gi.input_current >= gp.Outcome_MinVal AND gi.input_current<=gp.Outcome_Maxval AND gp.Outcome_IsActive=0 ORDER BY gp.Outcome_Order";

$object = $functionsObj->ExecuteQuery($sql);

$url       = site_root."input.php?Scenario=".$result->Link_ScenarioID;

include_once doc_root.'views/output_anjana.php';
