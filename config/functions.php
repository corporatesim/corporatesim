...
class Functions extends database{
	...
	function UpdateGame($tblName, $array, $fieldName, $fieldvalue, $print_flag=0)
	{
		...
		$old_value = explode(',',$new_value); // converting to array
		$old_value = array_splice($old_value, 0,-2);
		$old_value = implode(',', $old_value); // converting to string
	...
	function generateOrderId(){
		$object  = $this->SelectData(array("(MAX(order_id) + 1) as order_id"), orders, array(), '', '', '', '', 0);
	...
	function getPizzaOptionDetails($item_id){
		$object = $this->SelectData(array(), food_optional_details, array("item_id=".$item_id),'','','','', 0);
		$array  = array();
		while($row = $object->fetch_object()){
			$obj      = $this->SelectData(array(), categories, array("category_id=".$row->category_type),'','','','', 0);
			...
			$obj = $this->SelectData(array(), subcategories, array("scat_id in (".$row->optional_items.")"),'','','','', 0);
			while($items = $obj->fetch_object()){
				array_push($array[$cat_name->category_id]["items"], array($items->scat_id,$items->scat_name,$items->price) );
			}
		}
		return $array;
	}
