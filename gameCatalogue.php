...
*/
	...
	else
	{
	 ...
	 //echo $UserId."<br>".$user_game; exit;
			...
			$sql="SELECT * FROM GAME_GAME gg WHERE gg.Game_Delete = 0 AND gg.Game_ID NOT IN( SELECT gug.UG_GameID FROM GAME_USERGAMES gug LEFT JOIN GAME_USERSTATUS gus ON gus.US_GameID = gug.UG_GameID AND gug.UG_UserID = gus.US_UserID WHERE gug.UG_UserID = $UserId AND( gus.US_LinkID < 1 OR gus.US_LinkID IS NULL ) )";
