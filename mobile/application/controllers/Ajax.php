...
class Ajax extends MY_Controller
{
		...
		foreach ($getComponent as $findLinkageSubRow) {
			...
			if ($findLinkageSubRow->SubLink_InputMode != 'none') {
				...
				if ($findLinkageSubRow->SubLink_InputMode == 'carry') {
					if ($findLinkageSubRow->SubLink_LinkIDcarry > 1 && $findLinkageSubRow->SubLink_CompIDcarry > 1) {
						$carrySql = "SELECT gi.input_current FROM GAME_INPUT gi WHERE gi.input_sublinkid=(SELECT gls.SubLink_ID FROM GAME_LINKAGE_SUB gls WHERE gls.SubLink_LinkID=$findLinkageSubRow->SubLink_LinkIDcarry AND gls.SubLink_CompID=$findLinkageSubRow->SubLink_CompIDcarry) AND gi.input_user=" . $this->session->userdata['botUserData']->User_id;
						$carryVal = $this->Common_Model->executeQuery($carrySql);

						$returnHtml .= "<div class='" . $showOtucome . "' $fontStyle><label>" . $findLinkageSubRow->SubLink_LabelCurrent . "</label><input type='text' data-sublinkid='" . $findLinkageSubRow->SubLink_ID . "' value='" . $carryVal[0]->input_current . "' readonly></div>";
					} else {
	...
	public function showLeaderboard($gameId = NULL)
	{
		...
		$leaderboardData = $this->Ajax_Model->fetchRecords('GAME_LEADERBOARD', $leadWhere, '', '', '', '');
		...
		if (count($leaderboardData) > 0) {
			...
			$leadData = $this->Ajax_Model->executeQuery($leadSql);
			...
			die(json_encode(["status" => 201, "csrfHash" => $this->csrfHash, "message" => "Leaderboard will be available asap."]));
		}
	}
}
