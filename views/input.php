...
$debug = false; // make it true to shwo the alert at user side
...
<div class="leftsideNav rotateCompAnti row">
  ...
  <?php if ($result->Link_IntroductionLink < 1) { 
    $introductionText      = ($result->Link_IntroductionText) ? $result->Link_IntroductionText : 'Introduction';
    $introductionColorCode = ($result->Link_IntroductionColorCode) ? $result->Link_IntroductionColorCode : 'lightskyblue';
    // echo $introductionText.' : and : '.$introductionColorCode;
  ?>
    ...
    <a title="This will open in new tab" class="" target="_blank" href="<?php echo $gameurl; ?>" id="introduction"><button style="background-color: <?php echo $introductionColorCode; ?>; color:white;" class="btn btn-primary"><?php echo $introductionText; ?></button></a>
  <?php } ?>

  <?php if ($result->Link_DescriptionLink < 1) { 
      $descriptionText      = $result->Link_DescriptionText ? $result->Link_DescriptionText : 'Description';
      $descriptionColorCode = $result->Link_DescriptionColorCode ? $result->Link_DescriptionColorCode : 'orange';
  ?>
    ...
    <a title="This will open in new tab" class="" target="_blank" href="<?php echo $scenurl.'&close=true'; ?>" id="description"><button style="background-color: <?php echo $descriptionColorCode; ?>; color:white;" class="btn btn-primary"><?php echo $descriptionText; ?></button></a>
  <?php } ?>
...
<script type="text/javascript">
  ...
  console.log("scenarioName: <?php echo $result->Scenario.' forLinkId '.$result->Link_ID; ?>");
...
<script type="text/javascript">
  ...
  if ($objsql_timer->num_rows > 0) {
    ...
    echo "console.log('Existing Timer: ".$min." remaining.');";
  ...
  <?php
    ...
    echo "console.log('Creating Timer: ".$min." from admin.');";
  ...
  function element_not_found(key) {
    ...
    if (input_field_keys[find] === undefined) {
      ...
      try
      {
        input_field_keys[find].split('_')
      }
      catch(e)
      {
        // console.log(e.message);
        // Cannot read property 'split' of undefined
        findcompsubcompformula(key, sql, e.message);
      }
    }
  ...
  function findCountZero() {
    ...
    $.ajax({
      ...
      error: function(jqXHR, exception) {
        ...
        // Swal.fire({
        //   icon: 'error',
        //   html: 'Some error occured while playing simulation. Please contact administrator.',
        //   showCancelButton: true,
  ...
  function findcompsubcompformula(type, query, errMsg)
  {
    ...
    fetch('includes/ajax/ajax_update_execute_input.php', {
      method: 'post',
        ...
        body: 'action=findFormulaForNone&gameid=<?php echo $result->Link_GameID;?>&scenid=<?php echo $result->Link_ScenarioID;?>&query='+query+'&linkid=<?php echo $result->Link_ID;?>&errorMsg='+errMsg+'&element='+type
