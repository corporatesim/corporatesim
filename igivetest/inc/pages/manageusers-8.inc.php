...
if(isset($_POST["box_users"]) && is_array($_POST["box_users"])) {
 ...
 array_push($f_userids, (int)$f_userid);
}
} else if(isset($_GET["userids"]) && $_GET["userids"] != "") {
	$f_userids = explode(SYSTEM_ARRAY_ITEM_SEPARATOR, readGetVar('userids'));
foreach($f_userids as $key=>$val)
 $f_userids[$key] = (int)$val;
} else {
	array_push($f_userids, (int)readGetVar('userid'));
}
...
if($i_rSet2->fields['groupid'] == SYSTEM_GROUP_MAX_INDEX)
 echo '<tr vAlign=top><td class=rowhdr2 colspan=7><img src="images/1x1.gif" width=1 height=1></td></tr>';
$i_counter++;
$i_pagewide_id++;
$i_rSet2->MoveNext();
}
