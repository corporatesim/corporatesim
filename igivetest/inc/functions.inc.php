...
global $DOCUMENT_ROOT, $DOCUMENT_PHPMAILER, $srv_settings;
include_once($DOCUMENT_PHPMAILER.'class.phpmailer.php');
$mail = new PHPMailer();
$mail->PluginDir = $DOCUMENT_PHPMAILER;
if(!empty($srv_settings['email_mode']) && ($srv_settings['email_mode'] == IGT_EMAIL_SENDING_MODE_SMTP)) {
 $mail->IsSMTP();
$mail->Host = $srv_settings['email']['host'];
$mail->SMTPAuth = $srv_settings['email']['smtp_auth'];
$mail->Username = $srv_settings['email']['username'];
$mail->Password = $srv_settings['email']['password'];
}
$mail->FromName = '';
