            ...
            <div class="title">
              <div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
                ...
                <div class="clearfix mb-20">
                  <h5 class="text-blue">Edit Formula</h5>
                </div>
                ...
                <div class="col-12 col-sm-4 form-group">
                  ...
                  <input type="hidden" id="Cmap_Enterprise_ID" name="Cmap_Enterprise_ID" value="<?php echo $itemFormulaList[0]->Enterprise_ID; ?>">
                  <select class="form-control" disabled="">
                    <option><?php echo $itemFormulaList[0]->Enterprise_Name; ?></option>
                  </select>
                </div>
                <?php } else{ ?>
                  <input type="hidden" id="Cmap_Enterprise_ID" name="Cmap_Enterprise_ID" value="<?php echo $itemFormulaList[0]->Enterprise_ID; ?>">
                ...
                <div class="col-12 col-sm-4 form-group">
                  ...
                  <input type="text" name="formula_title" id="formula_title" class="form-control" required="" value="<?php echo $itemFormulaList[0]->Items_Formula_Title; ?>" />
                </div>
                ...
                <div class="formula_box">       
                  <div class="col-8 form-group">
                    ...
                    <textarea class="form-control" rows="5" readonly><?php echo $itemFormulaList[0]->Items_Formula_String; ?></textarea>
                  </div>    
                </div>
                <!-- end of Original formula box -->
                ...
                <div class="text-center">
                  <button class="btn btn-primary" name="" type="submit">Update Formula</button>
...
<script>
  $(document).ready(function(){
    ...
    let database_formula_expression = "<?php echo $itemFormulaList[0]->Items_Formula_Expression; ?>";
    let database_formula_string     = "<?php echo $itemFormulaList[0]->Items_Formula_String; ?>";
        //converting string to array
        database_formula_expression = database_formula_expression.split(" ");
        database_formula_string     = database_formula_string.split("} ");

    var formula_expression = [];
    var formula_string     = [];

    //setting formula expression in array and hidden field
    for(i=0; i<database_formula_expression.length; i++){
      formula_expression.push(database_formula_expression[i]);
      $('#formula_expression').val(formula_expression.join(" "));
    }

    //setting formula String in array and view
    for(i=0; i<database_formula_string.length; i++){
      if(i == database_formula_string.length-1)
        formula_string.push(database_formula_string[i]);
      else
        formula_string.push(database_formula_string[i]+"}");
    ...
    $('#itemFormulaForm').submit(function(e){
      ...
      if(formula_expression[0]){
        ...
        let formula_id = "<?php echo base64_decode($this->uri->segment(3)); ?>";
        var data = $("#itemFormulaForm").serialize()+'&formula_item_id='+formula_includes_items_id+'&formula_id='+formula_id;
          ...
          success: function(result){
            ...
            if(result.status == 200){
              Swal.fire({
                ...
                icon: result.icon,
                title: result.title,
                html: 'Formula Updated Successfully.',
    ...
    //according to Enterprise setting Performance type items wise
