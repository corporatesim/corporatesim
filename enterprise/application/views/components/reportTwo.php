          ...
          <div class="col-12 pb-5">
            <div class="title">
              <div class="pd-20 bg-white border-radius-4 box-shadow mb-30">
                ...
                <form action="" method="post" class="" id="fetchReportData">
                  ...
                  <div class="row col-md-12 col-lg-12 col-sm-12 row form-group" id="gameDiv">
                    <label for="selectCard" class="col-sm-12 col-md-3 col-form-label">Select Card</label>
                    <div class="col-sm-12 col-md-9">
                      <select name="selectCard" id="selectCard" class="custom-select2 form-control" required="">
  ...
  $(document).ready(function(){
    ...
    $('.getReport').on('click', function() {
      ...
      // } 
        ...
        let cardSelected = $('#selectCard').val();
        //console.log(cardSelected);

        let user_selected = $('#Cmap_UserId').val();

        if (cardSelected < 1 || cardSelected == '--Select Card--') {
          // Card not Selected
          // removing table if user not Selected Card
          $('#addTable').html('');

          $('#addTable').html('<span class="text-danger text-center">Please Select Card To View Report</span>');
        } 
        else {
          // show wait text to users
          $('#addTable').html('<span class="text-success text-center">Please Wait While Loading Report</span>');

          // tirgger ajax and get the user report
          var formData = $('#fetchReportData').serialize();
          $.ajax({
            url: "<?php echo base_url('AjaxNew/getReportTwoDataTable/'); ?>",
            type: 'POST',
            data: formData,
            ...
            success: function(result) {
              ...
              if (reportData.status == 201) {
                Swal.fire({
                  ...
                  html: reportData.message,
                  showConfirmButton: reportData.showConfirmButton,
                  timer: reportData.timer,
                });
              } 
              else if (reportData.status == 200) {
                // removing the previous chart report if user viewed earlier
                $('#addTable').html(reportData.data);
                makeTableDataTable();
              }
            }
          });
