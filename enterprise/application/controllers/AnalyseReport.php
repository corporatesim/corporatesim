...
class AnalyseReport extends CI_Controller {
		...
		if($RequestMethod == 'POST')
		{
			...
			else
			{
				switch ($filtertype)
				{
					...
					redirect(base_url('AnalyseReport/viewReport/'.$redirect));
					break;
	...
	public function viewReport($reportData=NULL)
	{
		...
		$dateTo        = (isset($reportData[1]))?date('Y-m-d H:i:s',$reportData[1]):NULL;
		$dateFrom      = (isset($reportData[2]))?date('Y-m-d H:i:s',$reportData[2]):NULL;
